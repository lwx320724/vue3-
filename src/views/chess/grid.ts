...
const verification = (data) => {
    ...
    for (let i = 0; i < data.length; i++) {
        let item = data[i].slice().sort((a, b) => a - b)
        if (JSON.stringify(item) != JSON.stringify([1, 2, 3, 4, 5, 6, 7, 8, 9])) return false
    }
    //判断每列
    for (let i = 0; i < data.length; i++) {
        let item = []
        for (let j = 0; j < data.length; j++) {
            item.push(data[j][i] * 1)
        }
...
export default { findAnswer, verification }